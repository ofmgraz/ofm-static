document.addEventListener("DOMContentLoaded", function () {
  ...
  function parseZonesFromXML(xmlString) {
    ...
    if (!targetZone) {
        // Try finding zone referenced by ab element
        const abElement = xmlDoc.querySelector(`lb[facs='#${hash}']`);
        if (abElement) {
            const referencedZoneId = abElement.getAttribute('facs').substring(1);
            for (let zone of zones) {
    ...
    // Find all inner zones within the target zone
    const innerZones = Array.from(targetZone.getElementsByTagNameNS("*", "zone"));
    innerZones.forEach(zone => {
        const zonePoints = zone.getAttribute("points").split(" ").map(point => {
            const [x, y] = point.split(",");
            return { x: parseInt(x, 10), y: parseInt(y, 10) };
        });
