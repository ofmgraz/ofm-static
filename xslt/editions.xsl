...
<xsl:stylesheet 
    ...
    <xsl:param name="mybreak"><![CDATA[<br/>]]></xsl:param><xsl:param name="mytab"><![CDATA[&emsp;]]></xsl:param>
    ...
    <xsl:template match="tei:lb">
        ...
        <xsl:if test="not(ancestor::tei:note[@type='footnote'])">
            <xsl:if test="ancestor::tei:p"><xsl:value-of select="$mybreak" disable-output-escaping="yes"/>
                <a>
                ...
                </a><xsl:value-of select="$mytab" disable-output-escaping="yes"/>
            </xsl:if>
        </xsl:if>
